...
function obtenerDatosUsuario() {
  ...
  if (usuario.estado !== "aceptado") {
    window.location.href = "../Pendiente/pendiente.html";
    return null;
  }
  ...
  const enlaceCerrarSesion = document.createElement("a");
  enlaceCerrarSesion.href = "../Login/Login.html";
  enlaceCerrarSesion.id = "cerrarSesion";
  enlaceCerrarSesion.innerHTML = `<div class="enlace">🚪</div>`;
  enlaces.appendChild(enlaceCerrarSesion);
...
async function inicializarGraficoEntregas() {
  try {
    ...
    const anios = [
      ...new Set(
        Object.values(entregasData)
          .flat()
          .map((e) => e.fecha.substring(0, 4)),
      ),
    ].sort();
    const selectAnio = document.getElementById("filtroAnioEntregas");
    anios.forEach(
      (a) => (selectAnio.innerHTML += `<option value="${a}">${a}</option>`),
    );
...
function renderizarGraficoEntregas(mes = null, anio = null) {
  ...
  const datasets = Object.keys(entregasData).map((nombre, index) => {
    ...
    if (anio)
      registros = registros.filter((e) => e.fecha.substring(0, 4) === anio);
    if (mes)
      registros = registros.filter((e) => e.fecha.substring(5, 7) === mes);
    return {
      label: nombre,
  ...
  const todasFechas = [
    ...new Set(
      Object.values(entregasData)
        ...
        .filter(
          (e) =>
            (!anio || e.fecha.substring(0, 4) === anio) &&
            (!mes || e.fecha.substring(5, 7) === mes),
        )
        .map((e) => e.fecha),
    ),
  ].sort();
...
async function inicializarGraficoPreciosProductos() {
  try {
    ...
    const anios = [
      ...new Set(
        Object.values(historialPrecios)
          .flat()
...
function renderizarGraficoPrecio(nombreProducto, mes = null, anio = null) {
  ...
  if (anio)
    registros = registros.filter((r) => r.fecha.substring(0, 4) === anio);
  if (mes)
    registros = registros.filter((r) => r.fecha.substring(5, 7) === mes);

  const datasets = [
    {
      label: "Congelado",
      data: registros.map((r) => r.precio_congelado),
      borderColor: "blue",
      fill: false,
      stepped: true,
    },
    {
      label: "M",
      data: registros.map((r) => r.precio_m),
      borderColor: "green",
      fill: false,
      stepped: true,
    },
    {
      label: "L",
      data: registros.map((r) => r.precio_l),
      borderColor: "orange",
      fill: false,
      stepped: true,
    },
    {
      label: "Jumbo",
      data: registros.map((r) => r.precio_jumbo),
      borderColor: "red",
      fill: false,
      stepped: true,
    },
